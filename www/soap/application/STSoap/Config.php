...
#      $cmd = 'invoke-rc.d networking stop 2> /dev/null; sleep 2; invoke-rc.d networking start 2> /dev/null && /etc/init.d/ssh restart 2> /dev/null && echo done.';
   ...
   */
        static public function Config_register($data) {
                ...
                require_once('SpamTagger/Config.php');
                $sysconf = SpamTagger_Config::getInstance();
                $cmd = $sysconf->getOption('SRCDIR')."/bin/register_SpamTagger.sh ".$data['resellerid']." ".$pass." ".$data['clientid']." -b ";
		$res = `$cmd`;
	...
	static public function Config_unregister($data) {
		...
		if (!isset($data['rsp']))
			$data['rsp'] = "--no-rsp";
                ...
                $cmd = $sysconf->getOption('SRCDIR')."/bin/unregister_spamtagger.sh ".$data['rsp']." -b";
                $res = `$cmd`;
                if (preg_match('/SUCCESS/', $res)) {
                   return 'OK registered '.$res;
                }
	...
	static public function Config_autoconfigurationDownload($data) {
			...
			if (file_exists($sysconf->getOption('SRCDIR').'/etc/autoconf/prepare_sqlconf.sh')) {
				...
				$res = `$cmd_prepare`;
				// When updating the st_config, need to restart ST
