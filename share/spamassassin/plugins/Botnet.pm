...
sub new ($class, $mailsa) {
  ...
  $this->register_eval_rule("botnet_nordns");
  $this->register_eval_rule("botnet_baddns");
  $this->register_eval_rule("botnet_ipinhostname");
  $this->register_eval_rule("botnet_clientwords");
  $this->register_eval_rule("botnet_serverwords");
  $this->register_eval_rule("botnet_soho");
  $this->register_eval_rule("botnet_client");
  $this->register_eval_rule("botnet");

  $this->{main}->{conf}->{botnet_pass_auth}    = 0;
  $this->{main}->{conf}->{botnet_pass_trusted} = "public";
  $this->{main}->{conf}->{botnet_skip_ip}      = "";
  $this->{main}->{conf}->{botnet_pass_ip}      = "";
  $this->{main}->{conf}->{botnet_pass_domains} = "";
  $this->{main}->{conf}->{botnet_clientwords}  = "";
  $this->{main}->{conf}->{botnet_serverwords}  = "";

  return $this;
}


sub parse_config ($this, $opts) {
  my ($temp);
  my $key = $opts->{key};
  my $value = $opts->{value};
    ...
    ($key eq "botnet_pass_trusted") )
  {
    ...
    $this->{main}->{conf}->{$key} = $value;
    $this->inhibit_further_callbacks();
    ...
    || ($key eq "botnet_serverwords") )
  {
    foreach my $temp (split(/\s+/, $value)) {
      ...
      Mail::SpamAssassin::Plugin::dbg("Botnet: adding " . $temp . " to $key");

      if ($this->{main}->{conf}->{$key} ne "") {
        $this->{main}->{conf}->{$key} =
          $this->{main}->{conf}->{$key} . "|(" . $temp . ")";
      } else {
        $this->{main}->{conf}->{$key} = "(" . $temp . ")";
      }
    }
...
sub _botnet_get_relay ($this, $pms) {
  ...
  my $skip_ip = $this->{main}->{conf}->{botnet_skip_ip};
  my $pass_ip = $this->{main}->{conf}->{botnet_pass_ip};
  my $pass_trusted = $this->{main}->{conf}->{botnet_pass_trusted};
  my $pass_auth = $this->{main}->{conf}->{botnet_pass_auth};
  my $pass_domains = '(?:' . $this->{main}->{conf}->{botnet_pass_domains} . ')';
    ...
    ($pass_trusted eq "private")) )
  {
    foreach my $relay (@trusted) {
        ...
        ($relay->{ip} =~ /$private_ips/) )
      {
        ...
        return (0, "", "", "");
      } elsif ( ($pass_trusted eq "public") && ($relay->{ip} !~ /$private_ips/) ) {
  ...
  while (1) {
    ...
    if (! defined ($ip) ) {
        ...
        ($relay->{rdns} ne "-1"))
      {
        ...
        Mail::SpamAssassin::Plugin::dbg("Botnet: get_relay didn't find RDNS");
...
sub botnet_nordns ($this, $pms) {
  my ($code, $ip, $helo);
  my $hostname = "";

  Mail::SpamAssassin::Plugin::dbg("Botnet: checking NORDNS");

  ($code, $ip, $hostname, $helo) = $this->_botnet_get_relay($pms);
  unless ($code) {
...
sub botnet_ipinhostname ($this, $pms) {
  my ($code, $ip, $helo);
  my $hostname = "";
...
sub botnet_clientwords ($this, $pms) {
  my ($code, $ip, $helo);
  my $hostname = "";
  my $wordre = $this->{main}->{conf}->{botnet_clientwords};
...
sub botnet_serverwords ($this, $pms) {
  my ($code, $ip, $helo);
  my $hostname = "";
  my $wordre = $this->{main}->{conf}->{botnet_serverwords};
...
sub botnet_client ($this, $pms) {
  my ($code, $ip, $helo);
  my $hostname = "";
  my $cwordre = $this->{main}->{conf}->{botnet_clientwords};
  my $swordre = $this->{main}->{conf}->{botnet_serverwords};
  my $tests = 0;
...
sub botnet ($this, $pms) {
  ...
  if (
    (defined ($sender = $pms->get("EnvelopeFrom"))) ||
  ...
  if (
    check_botnet($hostname, $ip, $cwordre, $swordre,
    $domain, $helo, \$tests)) 
  {
...
sub check_client ($hostname, $ip, $cwordre, $swordre, $tests = undef) {
...
sub check_botnet ($hostname, $ip, $cwordre, $swordre, $domain, $helo, $tests = undef) {
  ...
  if (
    ($baddns || $client) &&  # only run soho check if necessary
    (check_soho($hostname, $ip, $domain, $helo)) )
  {
    # looks like a SOHO mail server
...
sub check_soho ($hostname, $ip, $domain, $helo) {
...
sub check_dns ($name, $ip, $type, $max) {
