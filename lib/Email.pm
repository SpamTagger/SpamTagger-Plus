...
sub loadedIsWWListed {
  ...
  if ($db && $db->ping()) {
    ...
    foreach my $listedsender (@senders) {
      return 2 if (Email::listMatch($listedsender->{'sender'}, $sender));
    }

    $query = "SELECT sender FROM wwlists WHERE recipient='' AND type='$type' AND status=1";
    @senders = $db->getListOfHash($query);
    foreach my $listedsender (@senders) {
      return 1 if (Email::listMatch($listedsender->{'sender'}, $sender));
    }
  }
  return 0;
}
...
sub inWW {
  ...
  if ($type eq 'whitelist') {
     return 1 if ($prefclient->isWhitelisted($destination, $sender));
     return 0;
  }
  return 1 if ($prefclient->isWarnlisted($destination, $sender));
  return 0;
}
...
sub getLinkedAddresses {
  ...
  $this->{user} = User::create($this->{address}) unless ($this->{user});
