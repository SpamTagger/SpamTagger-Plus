...
while [[ $INODES -gt 70 ]]; do
  ...
  if [[ $DONE -eq 0 ]]; then
    find /var/spamtagger/run/spamtagger/log_search -type f -mtime +30 -delete
    DONE=1
  fi
  # If we already dropped files older than 120 days, inform the administrator
  if [[ $DAYS -eq 60 ]]; then
    EMAIL=$(echo "select contact_email from system_conf" | st_mariadb -s st_config | grep -v 'contact_email')
  ...
  find /var/spamtagger/spool/spamtagger/counts -type f -mtime +$DAYS -delete
  DAYS=$((DAYS - 60))
done
