...
sub do {
  ...
  if ($this->isSTInstalled) {
    $yndlg->build("\nA previous installation of SpamTagger has been detected and will then be deleted!!\nAre you sure you want to continue ? ", 'yes');
    return if ($yndlg->display() eq 'no');

    ## remove any spamtagger related cron cron
    print "\n";
    print " - Removing scheduled jobs... ";
    `crontab -l | grep -v 'spamtagger' > /tmp/crontab.tmp`;
    `crontab /tmp/crontab.tmp`;
    print "[done]\n";

    print " - Shutting down SpamTagger services... ";
    `/etc/init.d/spamtagger stop 2>&1 > /dev/null`;
    # make sure everything is gone
    `killall -TERM mysqld_safe mysqld httpd snmpd exim MailScanner 2>&1 > /dev/null`;
    print "[done]\n";
   
    print " - Removing SpamTagger directories... ";
  ...
  $dlg->clear();
  print "SpamTagger installation\n";
  print "------------------------\n\n";

  $dlg->build('Enter the unique ID of this SpamTagger in your infrastucture', '1');
  my $hostid = '';
  while ( $hostid !~ m/^\d+$/ ) {
  ...
  print CONF "STHOSTNAME = ".$hostname."\n";
  ...
  print " - Unpacking SpamTagger files...                      ";
  chdir('/root');
  if ( -d 'spamtagger') {
    `rm -rf spamtagger`;
  }
  if ( -f 'spamtagger_install.tar.lzma') {
    `tar --lzma -xf spamtagger_install.tar.lzma`;
  } else {
    `tar -xzf spamtagger_install.tar.gz`;
  }
  if ( ! -d '/root/spamtagger' ) {
    print "HuHo ! Cannot unpack SpamTagger archive. Maybe try to reinstall SpamTagger.\n";
    return;
  }
  ...
  if (! -x $cmd) {
    print "HuHo ! Cannot find main installer script. Maybe try to reinstall SpamTagger\n";
    return;
  }
  print "[done]\n";
