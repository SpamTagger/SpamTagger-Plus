...
if [ -f /root/Updater4ST/updater_$(date +%Y-%m-%d).log ]; then
  log_file="/root/Updater4ST/updater_$(date +%Y-%m-%d).log"
else
  log_file="/root/Updater4ST/updater_$(date +%Y-%m-%d -d yesterday).log"
fi
error_result=$(grep \
...
if [[ -n $error_result && -z $merge_result ]]; then
  echo "File conflicts when updating SpamTagger" >$OUT_FILE
  my_own_exit "1"
fi
