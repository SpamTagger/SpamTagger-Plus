...
STATUSFILE=/var/spamtagger/spool/spamtagger/updater4mc.status
if [ -e $STATUSFILE ]; then
  STATUS="$(cat $STATUSFILE)"
  # If running for longer than an hour or exited without cleaning
  if [[ $STATUS == "Running" ]]; then
    if [ "$(expr $(stat -c %Y $STATUSFILE) + 3600)" -lt "$(date +%s)" ]; then
      echo "Updater4ST still running after 1 hour or exited without completing" >>$OUT_FILE
      my_own_exit "1"
    fi
  ...
  elif grep -q "Not a valid SpamTagger Installation: no conf file" <<<$(echo "$STATUS"); then
    my_own_exit "3"
  else
    echo "Updater4ST encountered error: $STATUS" >>$OUT_FILE
    my_own_exit "255"
  fi
fi
