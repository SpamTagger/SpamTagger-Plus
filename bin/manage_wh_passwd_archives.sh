...
if [ $1 == "add" ]; then
  FIELD=$(echo "SELECT IFNULL(wh_passwd_archives, 'null') FROM dangerouscontent" | st_mysql -m st_config | tail -n +2)
  if [[ "$FIELD" == "null" ]]; then
    echo "UPDATE dangerouscontent set wh_passwd_archives =  CONCAT('$2', IFNULL(wh_passwd_archives, ''));" | st_mysql -m st_config
  else
    echo "UPDATE dangerouscontent set wh_passwd_archives =  CONCAT('$2', '\n', IFNULL(wh_passwd_archives, ''));" | st_mysql -m st_config
  fi
  echo "$2 added"
  exit
fi
...
if [ $1 == "del" ]; then
  ...
  echo "UPDATE dangerouscontent set wh_passwd_archives =  '$FIELD';" | st_mysql -m st_config
  echo "$2 removed"
  exit
fi
